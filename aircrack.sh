...
for ((i = 1; i <= $1; i++)); do
	if [[ $i -ge 1 && $i -le 9 ]]; then
		docker run -d --name "worker-0$i" -u nhom13 -w /home/nhom13/crack_me -v crack_me:/home/nhom13/crack_me worker:latest aircrack-ng CrackMe-01.cap -w "chunk_0$i.txt";
	else
		docker run -d --name "worker-$i" -u nhom13 -w /home/nhom13/crack_me -v crack_me:/home/nhom13/crack_me worker:latest aircrack-ng CrackMe-01.cap -w "chunk_$i.txt";
	fi
done
